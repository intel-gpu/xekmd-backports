    ...
    AC_COMPILE_IFELSE(
        ...
        [
            AC_DEFINE(BPM_DMA_BUF_ATTACH_OPS_INVALIDATE_MAPPINGS_NOT_PRESENT, 1,
                [dma_buf_attach_ops has invalidate_mappings member])
        ],
        [
            AC_DEFINE(BPM_DMA_BUF_ATTACH_OPS_INVALIDATE_MAPPINGS_NOT_PRESENT, 0,
                [dma_buf_attach_ops has move_notify member instead])
        ]
    )
])
