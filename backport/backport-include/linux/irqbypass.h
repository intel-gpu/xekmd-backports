...
#ifdef BPM_IRQ_BYPASS_REGISTER_PRODUCER_EVENTFD_NOT_PRESENT
...
#elif defined(BPM_IRQ_BYPASS_REGISTER_PRODUCER_IRQ_NOT_PRESENT)
static inline int bpm_irq_bypass_register_producer(struct irq_bypass_producer *producer,
						   struct eventfd_ctx *eventfd, int irq)
{
	producer->irq = irq;

	return irq_bypass_register_producer(producer, eventfd);
}
#define irq_bypass_register_producer bpm_irq_bypass_register_producer
#endif

#endif /* _BACKPORT_IRQBYPASS_H */
