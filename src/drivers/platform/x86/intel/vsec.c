...
static bool vsec_driver_present(int cap_id)
{
	...
	case VSEC_CAP_TELEMETRY:
		return IS_ENABLED(CPTCFG_INTEL_PMT_TELEMETRY);
	case VSEC_CAP_WATCHER:
		return IS_ENABLED(CONFIG_INTEL_PMT_WATCHER);
	case VSEC_CAP_CRASHLOG:
		return IS_ENABLED(CPTCFG_INTEL_PMT_CRASHLOG);
	case VSEC_CAP_SDSI:
		return IS_ENABLED(CPTCFG_INTEL_SDSI);
	case VSEC_CAP_TPMI:
		return IS_ENABLED(CPTCFG_INTEL_TPMI);
	case VSEC_CAP_DISCOVERY:
		return IS_ENABLED(CPTCFG_INTEL_PMT_DISCOVERY);
	default:
		return false;
	}
}
