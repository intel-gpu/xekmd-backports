			       ...
			       struct xe_guc_buf buf, u32 num_dwords)
{
	...
	if (IS_ENABLED(CPTCFG_DRM_XE_DEBUG_SRIOV)) {
...
{
	/* XXX: preliminary */
	return IS_ENABLED(CPTCFG_DRM_XE_DEBUG_SRIOV) ?
...
static u32 pf_get_min_spare_dbs(struct xe_gt *gt)
{
	...
	return IS_ENABLED(CPTCFG_DRM_XE_DEBUG_SRIOV) ? 1 : 0;
}
...
#ifdef CPTCFG_DRM_XE_DEBUG_SRIOV
