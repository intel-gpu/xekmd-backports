...
static const char *log_unknown_component_prefix(u32 component)
{
	...
	WARN(IS_ENABLED(CPTCFG_DRM_XE_DEBUG), "LOG: unrecognized component %u.%u\n", class, type);
	switch (class) {
...
unrecognized:
	pci_WARN(pdev, IS_ENABLED(CPTCFG_DRM_XE_DEBUG),
...
static const char *log_sev_prefix(int cper_sev)
{
	...
	case CPER_SEV_INFORMATIONAL:
		return "";
	default:
		WARN(IS_ENABLED(CPTCFG_DRM_XE_DEBUG), "LOG: unknown severity %d\n", cper_sev);
		return "";
	}
}
...
#if IS_BUILTIN(CPTCFG_DRM_XE_KUNIT_TEST)
