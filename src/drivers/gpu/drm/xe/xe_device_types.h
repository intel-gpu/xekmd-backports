...
struct xe_device {
	...
	struct {
		...
		/** @clients.list: client list for eudebug discovery */
...
#if IS_ENABLED(CPTCFG_PRELIM_DRM_XE_EUDEBUG)
	/** @eudebug: connection list and globals for device */
	struct {
		/** @eudebug.lock: protects the list of connections */
		spinlock_t lock;

		/** @eudebug.list: list of connections, aka debuggers */
		struct list_head list;

		/** @eudebug.session_count: session counter to track connections */
		u64 session_count;

		/** @eudebug.ordered_wq: used to discovery */
		struct workqueue_struct *ordered_wq;

		/** @eudebug.discovery_lock: used for discovery to block xe ioctls */
		struct rw_semaphore discovery_lock;

		/** @eudebug.state: debugging functionality state */
		enum xe_eudebug_state state;

		/** @eudebug.attention_scan: attention scan worker */
...
struct xe_file {
	...
	/**
	 * @pid: pid for file handle, used to safely access process info
	 * in situations where xe file can outlive drm_file
	 */
	struct pid *pid;
	...
	struct {
		...
		struct {
			...
			struct xarray xa;
			/** @eudebug.metadata.lock: protects debug metadata xarray */
			struct mutex lock;
		} metadata;
	} eudebug;
#endif
